...
s_node * search(s_node *head, char *searchString){
    s_node * runner = head;
    while(runner->next->next != NULL){
        char *contentOfNext = runner->next->content;
        if(strcmp(contentOfNext, searchString) == 0){
          return runner;
        }
        runner = runner->next;
    }
...
void delete(s_node *head, char *searchString){
    s_node * runner = head;
    while(runner->next->next != NULL){
        char *contentOfNext = runner->next->content;
        if(strcmp(contentOfNext, searchString) == 0){
          s_node * temp = runner->next;
